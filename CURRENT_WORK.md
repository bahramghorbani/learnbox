...
- **Learner Web Progress local truth (LB-DS-038):** implementation commit `3f111ae` labels daily count and streak as browser/device-local, shows the existing unacknowledged review-answer count only when non-zero, and states that server weekly history is inactive. Focused tests pass 18/18, full Website tests 229/229, typecheck/build and desktop/mobile 200%-text browser smoke pass. Draft PR, CI and independent review remain.
