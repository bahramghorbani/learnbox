...
- Checks unavailable: authenticated staging, axe and owner visual acceptance remain unverified. A real-Chromium independent probe found and blocked the initial shell-wide token override at `abe8a2a`; the follow-up restores the true-white canvas and legacy AA action colour, darkens tinted secondary text, and uses the D0 focus ring. Exact-head CI and review must rerun.
