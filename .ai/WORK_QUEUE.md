...
- **M2 Admin identity boundary:** ADR 0015 records that `admin_sessions.owner_singleton_id` cannot substitute for canonical `users.id`; server-backed review reads/writes remain fail-closed until an explicit foreign-key-constrained session-to-user binding is owner-approved, migrated and staging-verified.
