...
- Local evidence: implementation commit `d6541a3` on base `b49f325`; three focused test files produced genuine RED before any production edit (`Test Files 3 failed (3)`, `Tests 8 failed | 8 passed (16)`), re-confirmed against the stashed production edits, then GREEN 16/16; `pnpm --filter @learnbox/website test` 43 files/292 tests passed; website typecheck, website build (`/offline` prerendered static), `pnpm verify:security`, full `pnpm check`, Prettier and `git diff --check` all passed. Scope stayed inside the allowed paths: `sw.js` changed only `${CACHE_PREFIX}v8` to `v9`, and no copy claims sync or recovery of learner events. Independent exact-head review and seven CI contexts remain outstanding.
