...
- Head commit: `2e631b7`
- Draft PR: #251 — https://github.com/bahramghorbani/learnbox/pull/251 (draft; do not merge)
...
Implementation is complete in draft PR #251 at `2e631b7` and awaits independent high-reasoning security/data-integrity/product review. Migration 0017 ingests all 35 committed drafts deterministically (fixed uuid5 identities; content_json is the committed draft verbatim) with six pending checks each, guarded reruns that raise on divergent data, and no decision/reviewer/release values; the dormant `apps/api` content-review store and its test were moved/deleted (requirement 7) in favor of the extended Admin runtime store. The Admin queue/check/decision runtime is behind the dedicated default-off `LEARNBOX_ADMIN_CONTENT_REVIEW_ENABLED` gate (404 before any DB review read when disabled, session-only actor, DB role check before data access, no-store reads, origin/CSRF/recent-auth, row locks, idempotency fail-closed, atomic audit, no publication); `return_for_revision` now matches the canonical model (`needs_review`) instead of the dormant copy's `rejected`. The workspace is server-truthful when enabled and keeps an explicitly labeled local-only mode otherwise. Strict RED→GREEN evidence, migration validation against an ephemeral real Postgres (35/35/210 pending, idempotent rerun, fail-closed on divergence) and all final checks are recorded in `.ai/worker-reports/LB-DS-049.md`. Migration execution, flag activation and staging/Preview/Production remain unauthorized and untouched.
