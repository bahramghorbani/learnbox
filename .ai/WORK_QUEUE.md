...
- **M2 Admin identity boundary:** ADR 0015 records that `admin_sessions.owner_singleton_id` cannot substitute for canonical `users.id`; migration `0016` adds the approved nullable unique `admin_owner.user_id → users(id)` binding and session lookup now returns it fail-closed. Owner bootstrap, role assignment and staging verification remain gated before server-backed review reads/writes.
