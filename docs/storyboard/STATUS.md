...
- **Next:** add a server-side learner session before attaching private media to cards, then obtain owner approval before inviting any participant.
- **Blockers:** explicit owner approval is still required before any alpha invitation. Candidate
  ...
  media objects are private-storage-attested, but no server-side learner session exists to
  authorize card-media reads. The local machine has no Flutter SDK, but the latest GitHub CI
  mobile job passed.
