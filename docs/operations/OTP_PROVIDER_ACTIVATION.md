...
The required SMS.ir adapter is server-only and will use its `POST /v1/send/verify` endpoint with the `X-API-KEY` header, the owner-approved template ID, and a single code parameter. The browser must never receive those values. LearnBox's tested OTP core and database migration already cover opaque challenge lifecycle, five-minute expiry, one-minute resend cooldown, five attempts, keyed hashes and one-time consumption; only the persistent store adapter, routes and delivery adapter remain inactive.
