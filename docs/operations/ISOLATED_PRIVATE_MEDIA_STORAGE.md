...
| The uploader now requires the merged LB-DS-062 owner approval and exact isolated-store attestation before loading Blob write capabilities; mismatched and known-shared identities fail closed.                                        | `scripts/validate-private-media-upload-boundary.mjs`; `scripts/upload-start-slice-private-media.test.mjs`; PR #274 merge `63898012741335bb33f3ab4a8ad2fafe796c767b`                                  | The guard is implemented and verified, but execution still requires a separately selected owner-approved isolated target, owner-authenticated OIDC credentials and a distinct upload approval. |
...
- [x] LB-DS-062 tests missing, malformed, mismatched and known-shared attestation paths and proves zero Blob SDK capability load/write on rejection (merged in PR #274).
