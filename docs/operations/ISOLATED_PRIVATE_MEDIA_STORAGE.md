...
| Current learner containers expose private-media variable names but the server and public delivery flags are false; this read-only inspection found no populated Blob token in either inspected learner container. | `learnbox-prod` Docker inspection on 2026-09-11; names/presence only, no values logged | This is not proof of store isolation. It confirms no activation is being inferred from current runtime flags. |
