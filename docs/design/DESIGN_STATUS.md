...
| Profile / Settings         | Owner-approved bounded contract              | Web M3-P1 (`LB-DS-042`) adds Profile as the fourth persistent learner destination plus truthful Profile and child Settings surfaces from the device-local goal and pending-review queue with approved informational rows only; `M3_PROFILE_SETTINGS_CONTRACT.md` and PDR-006 record the approved fourth-tab navigation and fail-closed sign-out/deletion policies            | Partial           | M3-S1 sound preference; server identity reads (M3-A1); Android P1 parity and state-complete review |
