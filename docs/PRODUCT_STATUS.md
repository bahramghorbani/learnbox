...
| User offline tolerance              | Web / Android       | Verified foundation   | Local queue/recovery exists; truthful loading/error/offline labels on Today; Today label follows connectivity changes and re-reads learner state on reconnect (feature/learner-loop-completion); push-reconciliation cursor/watermark policy approved (ADR 0014); server implementation remains a serial M1-D queue task. Blocker for server-backed Today figures: reviewed Starter catalog seed (Start Pack ↔ canonical `contentId`) is an owner/review-gated decision |
...
| Content review workspace             | Verified foundation | Local review states and UI exist; the preview derives every claim from the committed Start Pack drafts, shows no passed validation/media/model-confidence for unreviewed drafts, and approve/return actions are preview-local only (publication stays blocked) |
...
| Content/media QA queue               | Gated               | Admin preview shows the complete Start Pack review queue and six-dimensional gate; server persistence and reviewer actions await canonical Admin session-to-user binding                                                                                       |
