...
LearnBox exposes an installable web manifest and a deliberately small service worker. It caches the public offline fallback page plus the approved launch image and installable icon; it does not cache authenticated API responses, phone numbers, personal cards, access tokens, or payment state. On an update, it removes only older LearnBox public-shell caches, never unrelated cache namespaces. Learner review events remain in the existing device queue and are synced only through the authenticated API when that boundary is available.
