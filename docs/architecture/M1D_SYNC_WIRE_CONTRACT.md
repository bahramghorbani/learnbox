...
Goal of this M1-D server slice and its future client composition: give a learner's
...
- **Server core cursor exists:** migration 0014
  ...
  rows report `'0'`. The dormant reconciliation GET now exposes only per-event cursor evidence.
...
- **Implemented server-side, dormant by default.** The route, flag gating, parser, learner-scoped
  query and tests exist. Client transport/composition and activation remain separate gated work.
...
### 3.2 Implemented dormant GET reconciliation
...
Response `200` — exact implemented shape, one top-level key (keeps the single-key transport
...
- `nextCursor` — exact cursor of the last event included in this page; equal to requested `cursor`
  when the page contains no events. It deliberately does not read a later learner cursor in a
  second statement, because a concurrent POST could otherwise advance the checkpoint past an event
  absent from this response.
- `hasMore` — page flag; `true` when more applied events exist beyond the fixed 100-event page cap.
...
Error responses (implemented, same taxonomy): 400 `{ "error": "validation" }` for malformed
...
- **Cursor domain:** non-negative integer, decimal string over the wire, BIGINT server-side
  ...
  metadata.
- **Cursor equality boundary (implemented):** events are "since `after`" when their stored
  per-event `review_events.reconciliation_cursor` is non-null and greater than `after`. Results are
  learner-scoped, ordered by that cursor and fetched as page-size + 1. The response checkpoint is
  derived from the last emitted row, preventing skips across pages even when a POST commits during
  a GET.
...
- **Implementation boundary:** the server GET exists and remains disabled by default. Client
  composition, any flag enablement and deployment remain separate serial tasks requiring review.
...
- **O-4 Page size:** server read currently uses a fixed 100-event page and fetches one extra row to
  derive `hasMore`; changing the cap remains a separate contract decision.
...
- **Rollout:** the server read is dormant behind the existing
